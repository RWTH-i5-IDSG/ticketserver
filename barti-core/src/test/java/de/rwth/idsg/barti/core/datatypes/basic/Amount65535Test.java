...
public class Amount65535Test {
    ...
    private void readTest(final byte[] bytes, final int expected) throws IOException {
        final Amount65535 read1 = Amount65535.READ_DESCRIPTION.read(new MyInputStream(new ByteArrayInputStream(bytes)));
        final int value1 = read1.getValue().getValue();
        assertEquals(expected, value1);
        final Amount65535 read2 = Amount65535.READ_DESCRIPTION.read(bytes);
        final int value2 = read2.getValue().getValue();
    ...
    private void writeTest(final int value, final byte[] expected) throws IOException {
        ...
        try (final MyOutputStream myOutputStream = new MyOutputStream(byteArrayOutputStream)) {
            new Amount65535(value).write(myOutputStream);
        }
    ...
    public void getLength() throws Exception {
        assertEquals(2, new Amount65535(0).getLength());
    }

    @Test
    public void getValue() throws Exception {
        for (final TestData testData : TEST_DATA) {
            assertEquals(testData.value, new Amount65535(testData.value).getValue().getValue());
        }
    }
}
